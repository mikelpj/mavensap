$(document).ready(function(){
      ...
      function getAreaName(areaNo) {
		...
		function query(cond){	
			...
			var url=context_path+'/wholesalereport/query.do';
		...
		function showDataList(datalist){
			...
			for(var i=0;i<datalist.length;i++){
				html+="<ul class='row'><li class='w20'>"+(i+1)+".</li><li class='w100'>"+getAreaName(datalist[i].eparchyName)+"</li>"
					   +"<li class='w300'>"+datalist[i].productName+"</li>"					   
					   +"<li class='w120'>"+datalist[i].flow+"</li>"
					   +"<li class='w120'>"+datalist[i].users+"</li>"
					   +"</ul>";
			}
			if(datalist.length<=0){
				html+="<ul class='row'><li class='w10'></li><li style='text-align:center;color:red;'>未查询到数据！<li></ul>"  ;
			}
			//html+="<div id="pager"></div><div style="clear:both;">";
		...
		function loadProductList(){	
			var url=context_path+'/product/queryProduct.do';	
			var data={
				state:'U'
			};		
			var callback=function(result){
